...
nlp.add_pipe("newline_cap_split", before="parser")
...
class LLMAttributionResult():
    ...
    def normalize_sum_to_one(self, attriubtion_matrix, norm: Literal["full", "clamp", "none"] = "full",) -> torch.Tensor:
        ...
        if norm == "full":
            ...
            # we add a small epsilon to ensure division by zero is impossible
    ...
    def normalize_max(self, attribution_vector) -> torch.Tensor:
        # divide by a max value > 0 
        # this is for a vector that we want to put on the range [X, 1]
        # we add a small epsilon to ensure division by zero is impossible
        if attribution_vector.max() > 0:
            attribution_vector = attribution_vector / (attribution_vector.max() + 1e-8)
        # divide by a minimum value < 0
        # this is for a vector that we want to put on the range [-1, X]
        # we add a small epsilon to ensure division by zero is impossible
        elif attribution_vector.max() <= 0:
            attribution_vector = -1 * attribution_vector / (attribution_vector.min() - 1e-8)
